...
src_install() {
	...
	use bundled-jre && fperms 755 "${dir}"/jre64/bin/{jaotc,java,javac,jdb,jjs,jrunscript,keytool,pack200,rmid,rmiregistry,serialver,unpack200}

	make_wrapper "${PN}-${SLOT}" "${dir}/bin/${BASENAME}.sh"
	newicon "bin/${BASENAME}.png" "${PN}-${SLOT}.png"
	make_desktop_entry "${PN}-${SLOT}" "PyCharm Professional ${SLOT}" "${PN}-${SLOT}" "Development;IDE;"
}
