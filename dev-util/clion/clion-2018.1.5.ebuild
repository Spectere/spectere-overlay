...
src_install() {
	...
	fperms 755 "${dir}"/bin/clang/clang-tidy
	fperms 755 "${dir}"/bin/cmake/bin/{ccmake,cmake,cpack,ctest}
	fperms 755 "${dir}"/bin/gdb/bin/{gcore,gdb,gdbserver}
	fperms 755 "${dir}"/bin/lldb/bin/{lldb,lldb-argdumper,LLDBFrontend,lldb-server}
	...
	make_desktop_entry "${PN}-${SLOT}" "CLion ${SLOT}" "${PN}" "Development;IDE;"
}
