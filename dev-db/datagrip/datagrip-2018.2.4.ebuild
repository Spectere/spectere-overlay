...
DESCRIPTION="A cross-platform IDE for databases and SQL."
...
src_prepare() {
	...
	# DataGrip only supports x86_64. Remove files that don't meet the criteria.
	local remove_files=(
		bin/fsnotifier
		bin/fsnotifier-arm
...
src_install() {
	...
	insinto "${dir}"
	doins -r *
	fperms 755 "${dir}"/bin/{{datagrip,format,inspect}.sh,fsnotifier64,{printenv,restart}.py}
	...
	make_desktop_entry "${PN}-${SLOT}" "DataGrip ${SLOT}" "${PN}-${SLOT}" "Development;Database;IDE;"
}
